...
一般在这个问题中，一般满足窗口大小**越短越合法**，并且一般都是以限制长度的最大值的条件，也就是**至少**的问题，因此我们窗口的状态大概是 合法 -> 不合法 -> 合法，如果合法就一直移动右指针 $right$ 使数组扩大，如果扩大之后使当前窗口不合法，那么就一直移动左指针直到窗口重新变成合法窗口，每次循环都记一次大小 $(res = res + (right - left + 1))$。
